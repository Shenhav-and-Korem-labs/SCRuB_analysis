...
finalized_simulations <- function(full_samps, 
  ...
  total_experiments <- n_experiments * 4 * 7 #3 * 4 * 4
    ...
    for(experiment in 1:n_experiments){
      ...
      for(plate_format in c('F')){ 
        ...
        for(well_level in c(0, .05, .25, .50)){
          ...
          if( (well_level == 0)|(contam_level==0.05) ){ # limiting the total number of simulations to what we show in the plots
...
alternative_datasets <- list.files('../data/Fig1/revision_datasets/')


for(path in alternative_datasets){
  sample_type <- ( path %>% str_split('_') )[[1]][1]
  
  synth_data <- read.csv(paste0('../data/Fig1/revision_datasets/', path), row.names = 1) %>% as.matrix()
  colnames(synth_data) <- paste0('OTU_', 1:ncol(synth_data))
  # synth_data <- synth_data %>% replace_na(0)
  controls <- synth_data[row.names(synth_data) %>% str_detect('BLANK') %>% 
                           which(), ]
  samples <- synth_data[( F == row.names(synth_data) %>% str_detect('BLANK') ) %>% 
                          which(), ]
  
  results_of_final_sims <- finalized_simulations(samples[samples %>% rowSums > 1000, ], 
                                                 controls[controls %>% rowSums > 150, ], 
                                                 n_experiments = 10
                                                 )
  
  results_of_final_sims %>% write_csv( (paste0( '../results/data/Fig_1_revised_simulations/', sample_type, '_results.csv') ) )
...
#   write.csv('../results/data/Fig_1_revised_simulations/F1_Simulation_Results.csv')
