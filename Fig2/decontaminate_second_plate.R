...
#scrub_only_out <-  SCRUB(samples =  P4[row.names(P4) %>% str_detect('extractNTC')==FALSE,] ,
#                         controls = P4[row.names(P4) %>% str_detect('extractNTC'),], 
...
dir.create('../results/data/Fig_2_alternate')

P4 %>% write.csv('../results/data/Fig_2_alternate/Raw.csv')
full_set_out$decontaminated_samples %>% write.csv('../results/data/Fig_2_alternate/SCRUB.csv')
decontam_out$decontaminated_samples %>% write.csv('../results/data/Fig_2_alternate/Decontam.csv')
decontam_lowbiomass_out$decontaminated_samples %>% write.csv('../results/data/Fig_2_alternate/Decontam_LB.csv')
restrictive_out %>% write.csv('../results/data/Fig_2_alternate/Restrictive.csv')
microdec_out$decontaminated_samples %>% write.csv('../results/data/Fig_2_alternate/microDecon.csv')


full_set_out$gamma %>% write.csv('../results/data/Fig_2_alternate/scrub_contaminant.csv')
1-decontam_out$cont_idx$p %>% write.csv('../results/data/Fig_2_alternate/decont_cont.csv')
decontam_lowbiomass_out$cont_idx$p %>% write.csv('../results/data/Fig_2_alternate/decontam_low_biomass_cont.csv')
(1 - microdec_out$decontaminated_samples /  md_setup[row.names(md_setup) %>% str_detect('extractNTC')==FALSE,] ) %>% colMeans(na.rm = T) %>% unname() %>%
  write.csv('../results/data/Fig_2_alternate/microdecon_cont.csv')

dir.create('../results/Supplementary_figures/')
dir.create('../results/Supplementary_figures/Well_to_well')
dir.create('../results/Supplementary_figures/Plasma_all')
